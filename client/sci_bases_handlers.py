...
class DocumentRetriever():
    ...
    async def get_files(self):
        ...
        await self.__get_pdf_file_if_exist()

        if self.pdf_file :
            self.ris_file = await self.__get_bilio_info()
    ...
    async def __get_bilio_info(self) -> dict:
        ...
        dwl_dir = AsyncPath(os.getenv("TEMP_EMPL"))
        files_in_temp = [f async for f in dwl_dir.iterdir() if await f.is_file()]
        if files_in_temp :
            ris_file = files_in_temp[0]
            ris_file_path = dwl_dir.joinpath(ris_file)
            async with aiofiles.open(ris_file_path, mode="r") as risfile:
                ris_content = await risfile.read()
            await aiofiles.os.remove(ris_file_path)
    ...
    async def __download_pdf_file(self):
        ...
        if files_in_temp :
            ...
            if pdf_file :
                ...
                with aiofiles.open(pdf_file, mode="rb") as pdf_local_file:
                    pdf = await pdf_local_file.read()
                lg.debug("Suppression du fichier pdf du dossier temporaire.")
                await aiofiles.os.remove(pdf_file)
                if pdf :
                    return pdf
                else:
                    return None
    ...
    async def __get_pdf_file_if_exist(self):
        ...
        if is_pdf :
            self.pdf_file = await self.__download_pdf_file()
        else :
...
class ScholarHandler():
    ...
    async def __get_result_page_files(self, url: Optional[str] | None, driver: Optional[Chrome] | None):
        ...
        for result in results :
            ...
            yield await extractor.get_files()
    ...
    async def __get_documents_from_research(self):
    ...
    async def search(self):
        ...
        await self.__get_documents_from_research()
